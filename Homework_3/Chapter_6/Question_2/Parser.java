...
class Parser {
  ...
  private Expr comma() {
    Expr expr = ternary(); // Updated to call ternary next
  ...
  // Ch 6 Challenge Question #2: Ternary operator ?:
  private Expr ternary() {
    Expr expr = equality();

    if (match(QUESTION)) {
      Token question = previous();
      Expr thenBranch = expression(); // Allows any precedence in the middle
      consume(COLON, "Expect ':' after ternary condition.");
      Token colon = previous();
      Expr elseBranch = ternary(); // Recurse to ternary for right-associativity
